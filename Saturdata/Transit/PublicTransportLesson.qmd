...
> We want to keep every transit stop,even in GEOIDS with multiple stops, meaning we mist use right_join()
...
> Remeber: Our data contains the variable `Geomotry` with the lat and long values of each stop

- Census Data Automatically Contains the `Geomotry`, or placement on a map, for each GEOID
...
ggplot(facet_data) +
  ...
  geom_sf_text(aes(label = stop_count), size = 2, color = "black") +
  scale_fill_viridis_c(option = "C", name = NULL) +
  facet_wrap(~ census_var, ncol = 2) +
  theme_minimal()
